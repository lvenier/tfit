...
Use `(P)ROFILE` on the main menu to open the profile screen after configuration. The profile screen has `(E)DIT`, `(V)IEW`, and the standard `(B)ACK` button. Use `Edit` to spell the current player name with the keyboard, `Enter` to save, `Backspace` to delete, and `Esc` to cancel. `View` shows the current selected player name in the compact profile panel. The app updates both the selected player profile and the stored face-profile name used in the panel. Punch Pad now uses the `(A)PUNCH PAD` shortcut so `(P)` can open Profile.
