...
When no face profiles are stored yet, Box4Fit automatically registers the currently selected player at startup. If a face is detected but does not match an existing profile, Box4Fit creates a new local player profile, selects it, and captures the same 5-8 sample registration set. To re-register, clear `localStorage.box4fit_face_profiles` and reload on the main menu. To disable first-run or unknown-face auto-registration, set `window.__TFIT_FACE_RECOGNITION_CONFIG__ = { autoRegisterWhenEmpty: false, autoRegisterUnknown: false }` before app startup.

Use `(P)ROFILE` on the main menu to open the profile screen after configuration. The profile screen has `(E)DIT`, `(V)IEW`, and the standard `(B)ACK` button. Use `Edit` to rename the current player and `View` to show the current selected player name in the compact profile panel. The app updates both the selected player profile and the stored face-profile name used in the panel. Punch Pad now uses the `(A)PUNCH PAD` shortcut so `(P)` can open Profile.
